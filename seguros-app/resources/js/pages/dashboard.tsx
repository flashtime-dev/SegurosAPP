...
export default function Dashboard({ comunidades, polizas, siniestros, rol }: { comunidades: number, polizas: number, siniestros: number, rol: Rol }) {
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="flex h-full flex-1 flex-col gap-4 rounded-xl p-4">
                <div className="grid auto-rows-min gap-4 md:grid-cols-3">
                    {/* Comunidades */}
                    {
                        rol.nombre === 'Superadministrador' || rol.permisos?.some(p => p.nombre === 'comunidades.ver') ? (
                            <Link href='/comunidades' prefetch>
                    ...
                    {/* Pólizas */}
                    {
                        rol.nombre === 'Superadministrador' || rol.permisos?.some(p => p.nombre === 'polizas.ver') ? (
                            <Link href='/polizas' prefetch>
                                <div className="bg-white dark:bg-gray-800 p-6 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm">
                                    <div className="flex items-center space-x-3">
                    ...
                    {/* Siniestros */}
                    {
                        rol.nombre === 'Superadministrador' || rol.permisos?.some(p => p.nombre === 'siniestros.ver') ? (
                            <Link href='/siniestros' prefetch>
                                <div className="bg-white dark:bg-gray-800 p-6 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm">
                                    <div className="flex items-center space-x-3">
